...
void UI::OnSave()
{
	...
	if (m_FilesPanel->GetCurrentTextureID() == FilesPanel::NO_CURRENT_TEXTURE_ID || !filepath)
		return;

	const Image* img = m_FilesPanel->GetCurrentImage();

	FileSystem::CopyFileX(User::GetLockScreenImageAbsolutePath(img->origName), *filepath);
}
...
void UI::Init()
{
	...
	ImGui_ImplGlfw_InitForOpenGL(m_App.GetWindow()->GetNativeWindow(), true);
	...
	io.Fonts->AddFontFromFileTTF("vendor/ImGui/misc/fonts/Roboto-Medium.ttf", 18.0f);
